...
- Project-scoped workspace members (server-side foundation — no UI yet): a workspace member can now optionally be restricted to one or more projects, reusing the exact `project: [[X]]` relation project-shares already resolve via `noteBelongsToProject`, rather than the previous all-or-nothing vault-wide access every member had. Zero scoping rows (today's only case) means nothing changes for any existing member. New `workspace_member_projects` table and `resolveEffectiveRole` (`server/auth.ts`) — the one place token/session/project-scope resolution now happens, called by both `server/index.ts`'s REST middleware and `server/collab.ts`'s real-time WebSocket handler. Closes a real gap the unification surfaced: the collab WS path previously called `resolveShareRole` directly, bypassing session/scoping entirely, so a project-scoped member could have opened the live-editing socket for any note outside their granted projects. Also makes `noteBelongsToProject`'s matching consistent with every other relation in the app (path-or-title, case-insensitive) instead of a bespoke exact-title-string comparison — a `project:` property can now reference its target by path, not just by title
