...
- Journal's live today entry looked broken, not Logseq-like: the focused block got a boxy `outline: 1px solid` border around it (a plain form-field look Logseq's own borderless inline editing never has — swapped for a soft background highlight instead), and the outliner itself wasn't wrapped in the same `.journal-entry-body` container every other entry uses, so it carried its standalone-editor padding/max-width instead and rendered double-indented and misaligned against the rest of the page
