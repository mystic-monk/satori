...
- **Chat with your notes**: ask a question, get an answer grounded in — and citing — the notes it's actually drawn from, powered by the same local embeddings Related Notes uses. Pick a local Ollama server (private, nothing leaves your machine) or a cloud API key you provide, entirely your call — see Roadmap for the same Node/browser-only scope Related Notes has
...
- Related Notes and Chat (both built on the same local semantic embeddings) only run in the Node/browser deployment — the Tauri native app needs its own Rust-side ML inference path (candle), not yet built, so neither appears there yet. Works best on notes with real paragraph content; very short one-liners give the embedding model less to work with. Chat also has no conversation memory across turns yet — each question is answered fresh off its own retrieval
